...
const cdnBase = 'https://cdn.jsdelivr.net/gh/' + repoOwner + '/' + repoName;
...
const domainJsonp = 'domainConfig(' + JSON.stringify(domainConfig, null, 2) + ');';
fs.writeFileSync(
...
tampermonkeyHeader += '// @description  Apply custom fonts and styles to various websites\n';
tampermonkeyHeader += '// @author       flinhong\n';
tampermonkeyHeader += '// @homepage     https://github.com/' + repoOwner + '/' + repoName + '\n';
tampermonkeyHeader += '// @supportURL   https://github.com/' + repoOwner + '/' + repoName + '/issues\n';
tampermonkeyHeader += '// @updateURL    ' + cdnBase + '/public/tampermonkey.js\n';
tampermonkeyHeader += '// @downloadURL  ' + cdnBase + '/public/tampermonkey.js\n';
...
userScriptBody += '    const configUrl = \'' + cdnBase + '/public/domain.jsonp\';\n';
...
userScriptBody += '        console.log(\'[Custom Font Styler] Config loaded:\', config.rules.length, \'rules\');\n';
userScriptBody += '        console.log(\'[Custom Font Styler] Current hostname:\', window.location.hostname);\n';
...
userScriptBody += '        if (!domainConfig || !domainConfig.rules) return null;\n\n';
userScriptBody += '        const fullUrl = window.location.href;\n\n';
userScriptBody += '        for (const rule of domainConfig.rules) {\n';
userScriptBody += "            const patterns = rule.domains || rule.match || [];\n";
userScriptBody += '            for (const pattern of patterns) {\n';
userScriptBody += '                const regex = matchPatternToRegex(pattern);\n';
...
userScriptBody += '                    console.log(\'[Custom Font Styler] Loaded:\', cssFile, \'for\', window.location.hostname);\n';
userScriptBody += '                })\n';
userScriptBody += '                .catch(err => {\n';
userScriptBody += '                    console.error(\'[Custom Font Styler] Failed to load:\', err);\n';
userScriptBody += '                });\n';
userScriptBody += '        } else {\n';
userScriptBody += '            const link = document.createElement(\'link\');\n';
userScriptBody += '            link.rel = \'stylesheet\';\n';
userScriptBody += '            link.href = cssUrl;\n';
userScriptBody += '            (document.head || document.documentElement).appendChild(link);\n';
userScriptBody += '        }\n';
userScriptBody += '    }\n\n';
...
userScriptBody += '                    console.error(\'[Custom Font Styler] Failed to load config:\', e);\n';
userScriptBody += '                }\n';
userScriptBody += '            },\n';
userScriptBody += '            onerror: function() {\n';
userScriptBody += '                console.error(\'[Custom Font Styler] Failed to fetch config\');\n';
userScriptBody += '            }\n';
userScriptBody += '        });\n';
userScriptBody += '    }\n\n';
