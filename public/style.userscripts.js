...
// @match        *://dns.frankindev.com/*
// @match        *://doh.frankindev.com/*
...
(function () {
  ...
  const BASE_URL =
    "https://cdn.honglin.ac.cn/statically/gh/flinhong/userscripts@v" +
    version +
    "/public";
  ...
  function loadCSS(file, css) {
    ...
    if (typeof GM !== "undefined" && GM.addStyle) {
      GM.addStyle(css)
        .then(() =>
          console.log("[Custom Styles] CSS loaded via GM.addStyle:", file),
        )
  ...
  function fallbackLinkInject(file, css) {
    try {
      ...
      console.error(
        "[Custom Styles] All injection methods failed for:",
        file,
        e,
      );
    }
    ...
    onload: function (response) {
      try {
        ...
        if (!text || typeof text !== "string" || !text.trim().startsWith("{")) {
          console.error(
            "[Custom Styles] Invalid config response:",
